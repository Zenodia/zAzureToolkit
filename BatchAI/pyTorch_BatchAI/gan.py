...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='pytorch cifar10 GAN example')
    parser.add_argument('-i', '--data', help = "The location of the processed datasets", required = True)
    parser.add_argument('--result', required=True,help='output resulting datasets')
    parser.add_argument('--epoch', required=True,  help='how many epoch')
    parser.add_argument('--model_path', required=True,  help='output path uploads model weights')
    ...
    #make sure the result folders as well as the data folders exist, otherwise make these folders
    ...
    # Loading the dataset
    dataset = dset.CIFAR10(root = args.data, download = True, transform = transform) # We download the training set in the ./data folder and we apply the previous transformations on each image.
    ...
    for epoch in range(int(args.epoch)): # We iterate over 25 epochs.
        ...
        for i, data in enumerate(dataloader, 0): # We iterate over the images of the dataset.
            ...
            print('[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f' % (epoch, args.epoch, i, len(dataloader), errD.data[0], errG.data[0])) # We print les losses of the discriminator (Loss_D) and the generator (Loss_G).
            if i % 100 == 0: # Every 100 steps:
                vutils.save_image(real, '%s/real_samples.png' % args.result, normalize = True) # We save the real images of the minibatch.
                fake = netG(noise) # We get our fake generated images.
                vutils.save_image(fake.data, '%s/fake_samples_epoch_%03d.png' % (args.result, epoch), normalize = True) # We also save the fake generated images of the minibatch.
    
    torch.save(netD, args.model_path+'netD.pt')
    torch.save(netG, args.model_path+'netG.pt')
    
        # Then later:
    netD = torch.load(args.model_oath+'netD.pt')
    netG = torch.load(args.model_oath+'netG.pt')